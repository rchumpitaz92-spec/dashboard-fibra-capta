...
# ============== DASHBOARD ==============
def render_dashboard(records, corte_records):
    ...
    if corte_records:
        ...
        injection = f'<script>window.PRELOADED_CORTE_DATA = {corte_json};</script>\n'
        html = html.replace(
            '<script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js',
            injection + '<script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js',
            1
        )
...
def main():
    ...
    if records is None:
        ...
        html = render_dashboard(records, corte_records)
        if html:
